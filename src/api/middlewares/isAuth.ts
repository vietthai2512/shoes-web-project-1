    ...
    jwt.verify(accessToken, JWT_ACCESS.SECRET, { algorithms: [JWT_ACCESS.ALGORITHM] }, (errAcc, decodedAcc) => 
    {
        if (errAcc)
        {
            jwt.verify(refreshToken, JWT_REFRESH.SECRET, { algorithms: [JWT_REFRESH.ALGORITHM] }, (errRef, decodedRef) =>
            {
                ...
                {
                    let _decodedRef = decodedRef as User;
                    const payload = {
                        id: _decodedRef.id,
                    ...
                    {
                        const [newAccessToken_HeaderPayload, newAccessToken_Signature] = splitToken(newAccessToken);
                        setTokenCookies(res, newAccessToken_HeaderPayload, newAccessToken_Signature, newRefreshToken);
                        next();
                    });
                }
            });
        ...
        {
            let _decodedAcc = decodedAcc as User;
            res.locals.isAuth = true;
